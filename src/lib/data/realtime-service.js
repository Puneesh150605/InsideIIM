...
export async function fetchRealTimeReport(query, horizon = '1-3y') {
    ...
    if (!quote) {
        try {
            ...
            if (results && results.quotes && results.quotes.length > 0) {
                ...
                const bestQuote = results.quotes.find((q) => (q.exchange === 'NSI' || q.exchange === 'BSE' || q.exchange === 'NMS' || q.exchange === 'NYQ') &&
                    (q.quoteType === 'EQUITY' || q.quoteType === 'MUTUALFUND' || q.quoteType === 'ETF')) || results.quotes.find((q) => q.quoteType === 'EQUITY') || results.quotes[0];
    ...
    if (debtToEquity < 35)
        score += 8;
    else if (debtToEquity < 80)
        score += 4;
    else if (debtToEquity > 180)
        score -= 14;
    else if (debtToEquity > 110)
        score -= 7;
    ...
    try {
        ...
        peerList = peerQuotes.filter(Boolean).map((pq) => ({
...
function formatMarketCap(cap, curr) {
    if (cap >= 1e12) {
