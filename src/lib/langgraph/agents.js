...
import { MARKET_DATA_PROMPT, COMPETITIVE_PROMPT, SENTIMENT_PROMPT, VALUATION_PROMPT, CIO_PROMPT } from './prompts';
function createLog(agent, message, status = 'completed') {
    return {
...
export async function marketDataNode(state) {
    ...
    const votes = [
        {
            ...
            vote: baseline.metrics.revenueYoY > 10 ? 'INVEST' : baseline.metrics.revenueYoY > 0 ? 'WATCH' : 'PASS',
        ...
        },
        {
            ...
            vote: baseline.metrics.roe > 15 ? 'INVEST' : baseline.metrics.roe > 8 ? 'WATCH' : 'PASS',
        ...
        },
        {
            ...
            vote: baseline.sentimentScore > 60 ? 'INVEST' : baseline.sentimentScore > 40 ? 'WATCH' : 'PASS',
        ...
        },
        {
            ...
            vote: baseline.metrics.targetPrice > baseline.metrics.currentPrice * 1.1 ? 'INVEST' : baseline.metrics.targetPrice > baseline.metrics.currentPrice * 0.95 ? 'WATCH' : 'PASS',
    ...
    if (llm) {
        try {
            ...
            if (parsed && parsed.decision && parsed.summary) {
                const dec = (parsed.decision === 'INVEST' || parsed.decision === 'PASS' || parsed.decision === 'WATCH') ? parsed.decision : baseline.decision;
                finalReport = {
                    ...baseline,
