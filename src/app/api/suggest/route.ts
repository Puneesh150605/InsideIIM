...
const POPULAR_INDIAN_GLOBAL: Record<string, { symbol: string; name: string; exch: string }> = {
    ...
    try {
      ...
      if (results && results.quotes) {
        for (const q of results.quotes) {
          ...
          const exch = String((q as any).exchange || (q as any).exch || 'MKT');
          const type = String((q as any).quoteType || 'EQUITY');
          const name = String((q as any).shortname || (q as any).longname || sym);

          // Filter out unwanted obscure derivative options or empty symbols
          if (type === 'OPTION' || type === 'CURRENCY') continue;
