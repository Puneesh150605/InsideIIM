...
export function LiveAgentVisualizer({ logs, company, isComplete }) {
    ...
    const getAgentStatus = (agentName) => {
        ...
        if (isDone && (isComplete || logs[logs.length - 1].agent !== agentName))
            return 'completed';
        return 'active';
    };
    ...
    return (<div className="w-full max-w-6xl mx-auto px-4 py-8 space-y-8">
      {/* Header */}
      ...
      <div className="glass-panel p-4 rounded-2xl border border-white/10 space-y-2">
        ...
        <div className="w-full h-2.5 bg-white/5 rounded-full overflow-hidden p-0.5 border border-white/10">
          <motion.div className="h-full bg-gradient-to-r from-indigo-500 via-purple-500 to-emerald-400 rounded-full" initial={{ width: 0 }} animate={{ width: `${progressPct}%` }} transition={{ duration: 0.5 }}/>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
        {AGENT_NODES.map((node, idx) => {
            ...
            return (<div key={node.id} className={`relative p-4 rounded-2xl border transition-all ${status === 'active'
              ...
              {idx < AGENT_NODES.length - 1 && (<div className="hidden md:block absolute -right-3 top-1/2 -translate-y-1/2 z-20 text-slate-500">
      ...
      <div className="glass-panel rounded-2xl border border-white/15 overflow-hidden shadow-2xl">
        ...
        <div ref={terminalRef} className="p-4 h-64 overflow-y-auto font-mono text-xs space-y-2 bg-[#06070a]">
          <AnimatePresence>
            {logs.map((log) => (<motion.div key={log.id} initial={{ opacity: 0, x: -5 }} animate={{ opacity: 1, x: 0 }} className={`flex items-start space-x-2 py-1 border-b border-white/[0.03] ${log.status === 'warning'
                ? 'text-amber-400'
                : log.status === 'active'
          ...
          {!isComplete && (<div className="flex items-center space-x-2 text-indigo-400 animate-pulse pt-2 font-semibold">
