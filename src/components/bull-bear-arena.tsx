...
import { TrendingUp, TrendingDown, AlertTriangle, ShieldCheck, Flame, Percent } from 'lucide-react';
...
export function BullBearArena({ report }: BullBearArenaProps) {
  return (
      ...
      <div className="glass-panel p-6 rounded-3xl border border-emerald-500/30 space-y-4 bg-gradient-to-b from-[#0e1714]/80 to-transparent">
        ...
        <div className="space-y-3 pt-1">
          {report.bullCase.map((arg, idx) => (
            <div
              key={idx}
              className="p-4 rounded-2xl bg-white/[0.03] hover:bg-white/[0.06] border border-white/5 transition-all space-y-2"
            >
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${
                    arg.impact === 'High' ? 'bg-indigo-500/20 text-indigo-300' : 'bg-slate-500/20 text-slate-300'
                  }`}>
                    {arg.impact} Impact
                  </span>
                </div>
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${
                    arg.impact === 'High' ? 'bg-amber-500/20 text-amber-300' : 'bg-slate-500/20 text-slate-300'
                  }`}>
                    {arg.impact} Impact
                  </span>
                </div>
