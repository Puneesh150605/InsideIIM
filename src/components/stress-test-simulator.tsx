...
import { ResearchReport, InvestmentDecision } from '../lib/types';

interface StressTestSimulatorProps {
  report: ResearchReport;
}

type ScenarioType = 'baseline' | 'rate_hike' | 'growth_boom' | 'recession';

export function StressTestSimulator({ report }: StressTestSimulatorProps) {
  ...
  const getSimulatedMetrics = () => {
    ...
    if (activeScenario === 'rate_hike') {
      ...
      const dec: InvestmentDecision = upside > 12 ? 'INVEST' : upside > 0 ? 'WATCH' : 'PASS';
      return {
        target: simTarget,
    ...
    if (activeScenario === 'growth_boom') {
      ...
      const dec: InvestmentDecision = upside > 10 ? 'INVEST' : 'WATCH';
      return {
        target: simTarget,
        upside: Math.round(upside * 10) / 10,
        score: simScore,
    ...
    if (activeScenario === 'recession') {
      ...
      const dec: InvestmentDecision = upside > 15 ? 'INVEST' : upside > -5 ? 'WATCH' : 'PASS';
      return {
        target: simTarget,
        upside: Math.round(upside * 10) / 10,
        score: simScore,
    ...
    <div className="glass-panel p-6 sm:p-8 rounded-3xl border border-white/10 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-white/10">
        ...
        <button
          ...
          className="self-start sm:self-auto flex items-center space-x-1 px-3 py-1.5 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-xs font-bold text-slate-300 transition-colors"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <button
          onClick={() => setActiveScenario('growth_boom')}
          className={`p-4 rounded-2xl border text-left transition-all ${
            activeScenario === 'growth_boom'
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-4 border-t border-white/10"
        >
          ...
          <div className="glass-card p-6 rounded-2xl border border-white/10 flex flex-col items-center justify-center text-center space-y-2 bg-black/40">
            ...
            <span className={`text-xs font-bold px-3 py-1 rounded-full border font-mono ${
              sim.upside >= 0 ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-rose-500/10 text-rose-400 border-rose-500/20'
            }`}>
