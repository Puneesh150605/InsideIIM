...
export function ApiKeyModal({ isOpen, onClose, provider, setProvider, apiKey, setApiKey, }) {
    ...
    return (<AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
        <motion.div initial={{ opacity: 0, scale: 0.95, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 10 }} className="relative w-full max-w-lg overflow-hidden rounded-2xl glass-panel border border-white/10 shadow-2xl p-6">
          {/* Header */}
          ...
          <div className="mt-5 space-y-3">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
              <button type="button" onClick={() => setLocalProvider('demo')} className={`flex flex-col items-center justify-center p-3 rounded-xl border text-center transition-all ${localProvider === 'demo'
            ...
            : 'bg-white/5 border-white/10 text-slate-300 hover:bg-white/10'}`}>
              ...
              <button type="button" onClick={() => setLocalProvider('gemini')} className={`flex flex-col items-center justify-center p-3 rounded-xl border text-center transition-all ${localProvider === 'gemini'
            ...
            : 'bg-white/5 border-white/10 text-slate-300 hover:bg-white/10'}`}>
              ...
              <button type="button" onClick={() => setLocalProvider('openai')} className={`flex flex-col items-center justify-center p-3 rounded-xl border text-center transition-all ${localProvider === 'openai'
          ...
          {/* API Key Input */}
          {localProvider !== 'demo' && (<motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="mt-5 space-y-2">
              ...
              <input type="password" value={localKey} onChange={(e) => setLocalKey(e.target.value)} placeholder={localProvider === 'gemini' ? 'AIzaSy...' : 'sk-...'} className="w-full px-4 py-2.5 rounded-xl glass-input text-sm placeholder:text-slate-500"/>
          ...
          {localProvider === 'demo' && (<div className="mt-5 p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-xs text-emerald-300 flex items-center space-x-2.5">
          ...
          <div className="mt-6 flex items-center justify-end space-x-3 pt-4 border-t border-white/10">
            <button type="button" onClick={onClose} className="px-4 py-2 text-xs font-semibold text-slate-400 hover:text-white transition-colors">
              Cancel
            </button>
            <button type="button" onClick={handleSave} className="flex items-center space-x-1.5 px-5 py-2.5 rounded-xl bg-gradient-to-r from-indigo-500 to-indigo-600 hover:from-indigo-600 hover:to-indigo-700 text-white font-semibold text-xs shadow-lg shadow-indigo-500/25 transition-all">
              {saved ? (<>
