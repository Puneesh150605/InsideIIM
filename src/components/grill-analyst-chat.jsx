...
export function GrillAnalystChat({ isOpen, onClose, report }) {
    ...
    return (<AnimatePresence>
      <div className="fixed inset-0 z-50 overflow-hidden bg-black/60 backdrop-blur-sm flex justify-end">
        <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', damping: 25, stiffness: 200 }} className="w-full max-w-md h-full bg-[#0d0f17] border-l border-white/10 shadow-2xl flex flex-col justify-between">
          {/* Header */}
          ...
          <div className="flex-1 p-4 overflow-y-auto space-y-4 text-xs">
            {messages.map((msg) => (<div key={msg.id} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                ...
                <div className={`p-3.5 rounded-2xl max-w-[88%] leading-relaxed ${msg.role === 'user'
            ...
            {isLoading && (<div className="flex items-center space-x-2 p-3 rounded-2xl glass-panel border border-white/10 max-w-[70%] text-slate-300">
          ...
          <div className="p-3 border-t border-white/5 bg-black/20 flex flex-wrap gap-1.5">
            {quickQuestions.map((q, idx) => (<button key={idx} onClick={() => setInput(q)} className="px-2.5 py-1 rounded-lg bg-white/5 hover:bg-indigo-500/20 border border-white/10 text-[11px] text-slate-300 hover:text-indigo-300 transition-all text-left">
                {q}
              </button>))}
          </div>
          ...
          <form onSubmit={handleSend} className="p-4 border-t border-white/10 bg-black/40">
            <div className="relative flex items-center">
              <input type="text" value={input} onChange={(e) => setInput(e.target.value)} placeholder="Ask analyst a question..." disabled={isLoading} className="w-full pl-4 pr-12 py-3 rounded-xl glass-input text-xs text-white placeholder:text-slate-500 disabled:opacity-50"/>
              <button type="submit" disabled={!input.trim() || isLoading} className="absolute right-1.5 top-1.5 bottom-1.5 px-3 rounded-lg bg-indigo-500 hover:bg-indigo-600 text-white flex items-center justify-center disabled:opacity-40 transition-all">
