...
import { InvestmentHorizon } from '../lib/types';
...
export function SearchSection() {
  ...
  const [horizon, setHorizon] = useState<InvestmentHorizon>('1-3y');
  ...
  const handleQuickClick = (ticker: string) => {
    setQuery(ticker);
    setShowDropdown(false);
    setIsSubmitting(true);
  ...
  return (
    <div className="relative py-16 sm:py-24 px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto text-center">
      {/* Glow Backdrop */}
      ...
      <motion.div
        ...
        className="inline-flex items-center space-x-2 px-3.5 py-1.5 rounded-full bg-white/5 border border-white/10 text-xs font-medium text-indigo-300 mb-6 backdrop-blur-md shadow-lg"
      >
      ...
      <motion.h1
        ...
        className="text-4xl sm:text-6xl lg:text-7xl font-black tracking-tight text-white font-sans leading-none mb-6"
      >
      ...
      <motion.div
        ...
        className="glass-panel p-3 sm:p-4 rounded-3xl shadow-2xl border border-white/15 max-w-3xl mx-auto backdrop-blur-2xl"
      >
        <form onSubmit={handleSearch} className="space-y-4">
          <div ref={searchContainerRef} className="relative flex items-center">
            ...
            <input
              ...
              placeholder="Enter company or ticker (e.g., Bajaj, Zomato, Reliance, Apple, TCS)..."
              disabled={isSubmitting}
              className="w-full pl-12 pr-32 py-4 rounded-2xl glass-input text-base sm:text-lg text-white placeholder:text-slate-500 font-medium disabled:opacity-50 transition-all focus:ring-2 focus:ring-indigo-500/50"
            />
            {isSuggesting && (
              <div className="absolute right-36 pointer-events-none">
            ...
            <AnimatePresence>
              {showDropdown && suggestions.length > 0 && (
                <motion.div
                  ...
                  className="absolute left-0 right-0 top-[64px] z-50 overflow-hidden rounded-2xl border border-white/20 bg-[#0c0f18]/95 p-2 shadow-2xl backdrop-blur-3xl text-left"
                >
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 pt-2 border-t border-white/10 px-2 text-xs text-slate-400">
            ...
            <div className="flex items-center space-x-2">
              {(['6-12m', '1-3y', '5y+'] as InvestmentHorizon[]).map((h) => (
                <button
                  key={h}
                  type="button"
      ...
      >
        ...
        <button
          onClick={() => handleQuickClick('NVDA')}
          disabled={isSubmitting}
          className="px-3 py-1.5 rounded-xl bg-white/5 hover:bg-emerald-500/15 border border-white/10 hover:border-emerald-500/40 text-slate-200 hover:text-emerald-300 font-semibold transition-all flex items-center space-x-1.5"
        >
        ...
        <button
          onClick={() => handleQuickClick('TCS')}
          disabled={isSubmitting}
          className="px-3 py-1.5 rounded-xl bg-white/5 hover:bg-indigo-500/15 border border-white/10 hover:border-indigo-500/40 text-slate-200 hover:text-indigo-300 font-semibold transition-all flex items-center space-x-1.5"
        >
        ...
        <button
          onClick={() => handleQuickClick('AAPL')}
          disabled={isSubmitting}
          className="px-3 py-1.5 rounded-xl bg-white/5 hover:bg-amber-500/15 border border-white/10 hover:border-amber-500/40 text-slate-200 hover:text-amber-300 font-semibold transition-all flex items-center space-x-1.5"
        >
