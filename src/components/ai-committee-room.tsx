...
import { Users, Award, TrendingUp, TrendingDown, ShieldAlert, CheckCircle2, Cpu, BarChart2, Shield, DollarSign } from 'lucide-react';
import { ResearchReport } from '../lib/types';

interface AiCommitteeRoomProps {
  report: ResearchReport;
}

export function AiCommitteeRoom({ report }: AiCommitteeRoomProps) {
  const intel = report.committeeIntelligence;
  if (!intel || !intel.votes || intel.votes.length === 0) return null;

  const getAgentIcon = (name: string) => {
    if (name.includes('Market')) return <BarChart2 className="w-5 h-5 text-indigo-400" />;
    if (name.includes('Competitive')) return <Shield className="w-5 h-5 text-emerald-400" />;
    if (name.includes('Sentiment')) return <Cpu className="w-5 h-5 text-purple-400" />;
    return <DollarSign className="w-5 h-5 text-amber-400" />;
  };

  const getVoteBadge = (vote: 'INVEST' | 'PASS' | 'WATCH') => {
    if (vote === 'INVEST') {
      return (
        <span className="inline-flex items-center space-x-1 px-2.5 py-1 rounded-full bg-emerald-500/20 text-emerald-300 font-bold text-xs border border-emerald-500/30">
    ...
    if (vote === 'PASS') {
      return (
        <span className="inline-flex items-center space-x-1 px-2.5 py-1 rounded-full bg-rose-500/20 text-rose-300 font-bold text-xs border border-rose-500/30">
    ...
    return (
      <span className="inline-flex items-center space-x-1 px-2.5 py-1 rounded-full bg-amber-500/20 text-amber-300 font-bold text-xs border border-amber-500/30">
  ...
  return (
    <div className="space-y-6">
              ...
              <div className="h-1.5 w-full bg-white/10 rounded-full overflow-hidden">
                <motion.div
                  ...
                  className={`h-full rounded-full ${
                    v.confidence >= 85 ? 'bg-emerald-400' : v.confidence >= 70 ? 'bg-indigo-400' : 'bg-amber-400'
                  }`}
                />
              </div>
            </div>
      ...
      <motion.div
        ...
        className="glass-panel p-6 rounded-2xl border border-indigo-500/40 bg-gradient-to-r from-indigo-900/20 via-purple-900/20 to-indigo-900/20 flex flex-col sm:flex-row items-center justify-between gap-6"
      >
        ...
        <div className="shrink-0 flex flex-col items-center justify-center p-4 rounded-xl bg-black/40 border border-white/10 min-w-[140px]">
          ...
          <span className={`text-2xl font-black mt-0.5 ${
            report.decision === 'INVEST' ? 'text-emerald-400' : report.decision === 'PASS' ? 'text-rose-400' : 'text-amber-400'
          }`}>
            {report.decision}
          </span>
        </div>
